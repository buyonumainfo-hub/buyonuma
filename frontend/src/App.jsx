...
const BuyerProtected = ({ children }) => {
  const { isAuthenticated, loading, authError, retryAuth } = useBuyerAuth();
  const [retrying, setRetrying] = useState(false);

  if (loading) return <Spinner />;

  // BUG FIX: mirrors SellerProtected's fix — a transient verify failure
  // (network blip, backend 500, rate limit) should offer a retry, not
  // silently redirect a validly-signed-in buyer to /buyer/login.
  if (authError) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--cream, #faf8f3)' }}>
        <LoadFailedModal
