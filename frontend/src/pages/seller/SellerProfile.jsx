...
import { User, Save, Upload, AtSign, Loader2 } from 'lucide-react';
...
const SellerProfile = () => {
  ...
  const [form, setForm] = useState({
    store_name: '', category: '', description: '', contact: '', whatsapp: '',
    website: '', social_media_handle: '', profile_picture: '', banner: '',
    state: '', city: '', address: '', showAddress: false,
  });
  const [uploading, setUploading] = useState({ profile: false, banner: false });
  const [loading, setLoading]     = useState(false);
  const [error, setError]         = useState('');

  // Username is edited separately (its own cooldown + JWT re-issue),
  // so it isn't part of the main profile form/submit above.
  ...
  const populateForm = (s) => {
    ...
    if (s.usernameChangedAt) {
      const next = new Date(new Date(s.usernameChangedAt).getTime() + 7 * 24 * 60 * 60 * 1000);
      setNextUsernameChangeAt(next > new Date() ? next : null);
    } else {
      setNextUsernameChangeAt(null);
    }
  };
  ...
  const handleUsernameSave = async () => {
    ...
    if (!clean || clean === seller?.username) return;
    setUsernameSaving(true);
    setUsernameError('');
    try {
      const res = await api.put('/seller-auth/username', { username: clean });
      // Username is embedded in the JWT, so the token itself changed —
      // store the fresh one before re-verifying, or every request after
      // this would still be authenticated as the old username.
      localStorage.setItem('lens_seller_token', res.data.token);
      await refreshSeller();
      toast.success('Username updated!');
    } catch (err) {
      const msg = err.response?.data?.message || 'Could not update username';
      setUsernameError(msg);
      if (err.response?.data?.nextAllowedAt) setNextUsernameChangeAt(new Date(err.response.data.nextAllowedAt));
      setUsername(seller?.username || username);
    } finally {
        ...
        <div className="seller-profile-card">
          ...
          <form onSubmit={handleSubmit} className="seller-profile-form">
            ...
            <div className="form-group">
              ...
              <input className="form-control" placeholder="e.g. 12 Allen Avenue, Ikeja" maxLength={200}
                value={form.address} onChange={e => set('address', e.target.value)} />
              <label className="seller-checkbox-row" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginTop: '0.4rem', fontSize: '0.8rem', color: 'var(--ink-muted)' }}>
                <input type="checkbox" checked={form.showAddress} onChange={e => set('showAddress', e.target.checked)} />
                Show this address publicly on my store page
              </label>
            </div>
        ...
        <div className="seller-readonly-card">
          ...
          <div className="readonly-rows">
            ...
            {nextUsernameChangeAt
              ? `You can change your username again on ${nextUsernameChangeAt.toLocaleDateString()}.`
            ...
            <div className="readonly-row">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                <div className="input-wrap" style={{ maxWidth: 200 }}>
                  <AtSign size={13} className="input-icon-left" />
                  <input
                    className="form-control"
                    style={{ fontSize: '0.82rem', padding: '0.4rem 0.6rem 0.4rem 1.7rem' }}
                    value={username}
                    disabled={Boolean(nextUsernameChangeAt) || usernameSaving}
                ...
                <button
                  ...
                  disabled={Boolean(nextUsernameChangeAt) || usernameSaving || !username.trim() || username === seller?.username}
                  onClick={handleUsernameSave}
                >
                  {usernameSaving ? <Loader2 size={13} className="spin" /> : 'Save'}
                </button>
              </div>
            </div>
          ...
          {usernameError && <p style={{ fontSize: '0.78rem', color: '#e0453c', marginTop: '0.5rem' }}>{usernameError}</p>}
